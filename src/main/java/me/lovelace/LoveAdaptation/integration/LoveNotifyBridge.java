package me.lovelace.LoveAdaptation.integration;
...
/**
 * Мост к LoveNotify (LoveCore). LoveAdaptation не подключает lovecore-api на этапе компиляции
 * ради одного интерфейса, поэтому, как и в LoveBehavior/LoveShop, интеграция сделана через
 * reflection поверх Bukkit ServicesManager — включая сам вложенный enum Channel (тоже неизвестен
 * на этапе компиляции). Если LoveCore/LoveNotify недоступен — канал считается разрешённым
 * (прежнее поведение "показывать всегда").
 */
public final class LoveNotifyBridge {

    private LoveNotifyBridge() {
    }

    /** {@code channelName} — "ACTION_BAR" или "TITLE" (имена констант LoveNotify.Channel). */
    public static boolean isChannelEnabled(JavaPlugin plugin, UUID uuid, String channelName) {
    ...
    private static Object findLoveNotifyApi() {
